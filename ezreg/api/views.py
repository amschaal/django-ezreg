...
class PriceViewset(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return Price.objects.filter(event__organizer__user_permissions__permission=OrganizerUserPermission.PERMISSION_ADMIN)
...
class PaymentProcessorViewset(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        return PaymentProcessor.objects.filter(organizer__user_permissions__permission=OrganizerUserPermission.PERMISSION_ADMIN)
...
class EventPageViewset(viewsets.ModelViewSet):
    ...
    search_fields = ('event',)
    def get_queryset(self):
        return EventPage.objects.filter(event__organizer__user_permissions__permission=OrganizerUserPermission.PERMISSION_ADMIN)
...
#                  }
    ...
    def get_queryset(self):
        return Registration.objects.filter(event__organizer__user_permissions__permission=OrganizerUserPermission.PERMISSION_VIEW)
...
#     queryset = MailerMessage.objects.all().prefetch_related('registrations')
    ...
    def get_queryset(self):
        return MailerMessage.objects.filter(registrations__event__organizer__user_permissions__permission=OrganizerUserPermission.PERMISSION_VIEW).prefetch_related('registrations')
"""
