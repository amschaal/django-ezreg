...
def email_status(registration):
    ...
    if registration.status == Registration.STATUS_APPLIED_DENIED:
        return send_email(to, from_addr, 'Your application has been declined for "%s"'%registration.event.title, 'ezreg/emails/application_denied.html', html_template='ezreg/emails/application_denied.html', context={'registration':registration}, bcc=bcc, registration=registration)
